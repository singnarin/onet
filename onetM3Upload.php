...
while (($objArr = fgetcsv($objCSV, 3000, ",")) !== FALSE) {
	...
	$sel_nt = mysql_query("SELECT * FROM `onetm3` WHERE `schoolid` = '".$objArr[0]."'");
	$count = mysql_num_rows($sel_nt);

	if($count == '1'){
		mysql_query("UPDATE `onetm3` SET `schoolID` = '".$objArr[0]."', `student` = '".$objArr[2]."' , `thai` = '".$objArr[3]."' , `social` = '".$objArr[4]."' , `english` = '".$objArr[5]."' , `math` = '".$objArr[6]."', `science` = '".$objArr[7]."', `average` = '".$objArr[8]."' WHERE `schoolid` = '".$objArr[0]."'");
	}else{
