...
while (($objArr = fgetcsv($objCSV, 3000, ",")) !== FALSE) {
	...
	$sel_nt = mysql_query("SELECT * FROM `onetm357` WHERE `schoolid` = '".$objArr[0]."'");
	$count = mysql_num_rows($sel_nt);

	if($count == '1'){
		mysql_query("UPDATE `onetm357` SET `schoolID` = '".$objArr[0]."', `student` = '".$objArr[2]."' , `thai` = '".$objArr[3]."' , `social` = '".$objArr[4]."' , `english` = '".$objArr[5]."' , `math` = '".$objArr[6]."', `science` = '".$objArr[7]."', `average` = '".$objArr[8]."' WHERE `schoolid` = '".$objArr[0]."'");
	}else{
		$sel_tbschool = mysql_query("SELECT * FROM `tbschool` WHERE `schoolid` = '".$objArr[0]."'");
		$count1 = mysql_num_rows($sel_tbschool);
			if ($count1 == '1') {
				$strSQL = "INSERT INTO onetm357 ";
